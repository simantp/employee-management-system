...
export default function StaffEmailLogsModal({
  ...
  // Retrieve all stored email logs for this employee
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-600 truncate mt-0.5 flex items-center gap-1.5 font-medium">
                ...
                <span className="text-slate-600 font-bold">
                  {staffLogs.length} Total Stored Email Record(s)
                </span>
              </p>
            </div>
          </div>
          ...
          {staffLogs.length === 0 ? (
            <div className="p-12 text-center space-y-3">
              ...
              <h4 className="font-bold text-slate-800 text-sm">
                No Email Records Stored for {employee.firstName} {employee.lastName}
              </h4>
              <p className="text-slate-500 max-w-sm mx-auto text-xs">
                Sent visa expiry reminders, driver license notices, and leave notifications for this employee will be stored and displayed here automatically upon dispatch.
              </p>
            </div>
          ) : (
            staffLogs.map((log) => {
