...
export default function PendingDocumentApprovals() {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-4 text-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {pendingDocs.map(({ doc, empId, empName, empNumber, dept }) => {
          ...
          return (
            <div 
              key={doc.id}
              className="p-4 rounded-2xl border border-amber-200/80 bg-amber-50/20 hover:bg-white hover:shadow-md transition space-y-3 flex flex-col justify-between"
            >
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                  <span className="px-2.5 py-1 bg-white text-slate-900 font-bold rounded-lg text-[10px] flex items-center gap-1 shadow">
                    <Eye className="w-3 h-3" />
                    <span>Inspect Document</span>
                  </span>
                </div>
                <div className="absolute top-2 right-2">
                  <span className="px-2 py-0.5 bg-amber-500 text-white rounded-full text-[9px] font-extrabold shadow-sm">
                    Pending
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-cyan-600 font-semibold truncate">{doc.type}</p>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold flex items-center justify-center gap-1 shadow-xs transition"
                >
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-xl font-bold border border-rose-200 transition"
              ...
              {rejectingDocId === doc.id && (
                <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl space-y-2 animate-in fade-in text-[11px]">
                  ...
                  <input
                    ...
                    className="w-full p-2 border border-rose-300 rounded-lg bg-white focus:outline-none"
                  />
                  <div className="flex justify-end gap-1.5">
                    <button
                      onClick={() => setRejectingDocId(null)}
                      className="px-2.5 py-1 border rounded-lg text-slate-600 font-bold bg-white"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      className="px-3 py-1 bg-rose-600 text-white font-bold rounded-lg shadow-xs"
                    >
                      Confirm Reject
                    </button>
                  </div>
      ...
      {selectedPreviewDoc && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/20 p-4" onClick={() => setSelectedPreviewDoc(null)}>
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-3 gap-3 p-3.5 bg-slate-50 rounded-2xl border border-slate-200">
                ...
                <div>
                  ...
                  <span className="font-bold text-cyan-600 text-xs">{selectedPreviewDoc.doc.type}</span>
                </div>
                <div>
