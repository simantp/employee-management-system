      ...
      <div className="flex items-center gap-4 flex-1 max-w-md">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition placeholder:text-slate-400 text-slate-800"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={notifRef}>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 w-5 h-5 bg-rose-500 text-white text-[10px] font-extrabold rounded-full flex items-center justify-center shadow-xs ring-2 ring-white animate-bounce">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {showNotifications && (
            <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white/95 backdrop-blur-xl border border-slate-200 rounded-3xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-150 text-xs">
              ...
              <div className="px-4.5 py-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
                ...
                {unreadCount > 0 && (
                  <button
                    ...
                    className="flex items-center gap-1 text-[11px] font-bold text-blue-600 hover:text-blue-800 transition py-1 px-2 rounded-lg hover:bg-blue-50"
                  >
                  ...
                  relevantNotifs.map(n => (
                    ...
                    >
                      <div className={`w-7 h-7 rounded-xl flex items-center justify-center flex-shrink-0 text-xs ${
                        n.type === 'LEAVE_REQUEST' || n.type === 'LEAVE_STATUS' ? 'bg-amber-100 text-amber-700' :
                        n.type === 'VISA_EXPIRY' ? 'bg-rose-100 text-rose-700' :
                        n.type === 'BANK_UPDATE' ? 'bg-cyan-100 text-cyan-700' :
                        'bg-blue-100 text-blue-700'
                      }`}>
                        {n.type === 'LEAVE_REQUEST' || n.type === 'LEAVE_STATUS' ? <Calendar className="w-3.5 h-3.5" /> :
                         n.type === 'VISA_EXPIRY' ? <ShieldAlert className="w-3.5 h-3.5" /> :
                         n.type === 'BANK_UPDATE' ? <CreditCard className="w-3.5 h-3.5" /> :
            ...
            {showUserDropdown && (
              <div className="absolute right-0 top-full mt-1.5 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl p-2 z-50 animate-in fade-in zoom-in-95">
                ...
                <button
                  ...
                  className="w-full text-left px-2.5 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 rounded-lg transition flex items-center gap-2"
                >
        ...
        {/* Primary Action Button (Admin Only: Add Employee) */}
        {isAdmin && onOpenAddModal && (
          <button
            onClick={onOpenAddModal}
            className="hidden sm:flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md shadow-blue-600/20 transition hover:shadow-lg"
          >
